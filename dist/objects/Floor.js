class Floor extends Shape {
    ...
    createObject(sizeX, sizeY, sizeZ) {
        ...
        let z2;
        let tileSize = 1;
        let maxLength = (Math.sqrt(sizeX * sizeX + sizeZ * sizeZ) / 3) * tileSize;
        const getY = (x, z) => {
        ...
        for (let x = 0; x < sizeX; x++) {
            ...
            x2 = x1 + tileSize;
            this
                .addTriangle(x1, y, z2, getUV(x), 0.5, x2, y, z2, getUV(x + 1), 0.5, x2, 0, z2, getUV(x + 1), 1)
                .addTriangle(x1, y, z2, getUV(x), 0.5, x2, 0, z2, getUV(x + 1), 1, x1, 0, z2, getUV(x), 1)
                .addTriangle(x1, y, z1, getUV(x + 1), 0, x1, 0, z1, getUV(x), 0, x2, 0, z1, getUV(x), 0.5)
                .addTriangle(x1, y, z1, getUV(x + 1), 0, x2, 0, z1, getUV(x), 0.5, x2, y, z1, getUV(x + 1), 0.5);
        }
        ...
        for (let z = 0; z < sizeZ; z++) {
            ...
            z2 = z1 + tileSize;
            this
                .addTriangle(x2, y, z1, 1, getUV(z), x2, 0, z1, 0.5, getUV(z), x2, 0, z2, 0.5, getUV(z + 1))
                .addTriangle(x2, y, z1, 1, getUV(z), x2, 0, z2, 0.5, getUV(z + 1), x2, y, z2, 1, getUV(z + 1))
                .addTriangle(x1, y, z1, 0.5, getUV(z), x1, y, z2, 0, getUV(z), x1, 0, z2, 0, getUV(z + 1))
                .addTriangle(x1, y, z1, 0.5, getUV(z), x1, 0, z2, 0, getUV(z + 1), x1, 0, z1, 0.5, getUV(z + 1));
        }
        this.createBuffers();
    }
}
